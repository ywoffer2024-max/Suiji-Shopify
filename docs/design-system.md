...
| 文档信息 | |
|---|---|
| 版本 | v2.2（组件落地与品牌资产定版，2026-06-12） |
...
| 9 | Logo | ✅ v2.2 | colorways 的 deep-charcoal 重着色为 ink `#1C1D21`、白底转透明 PNG；页眉 50px（Dawn 下限）。mint/金色与浅底网页冲突不用 |
| 10 | 页脚 | ✅ v2.2 落地 | scheme-3 深色四栏：品牌信息 / Explore / Support / Reach us（邮箱·WhatsApp·IG·TikTok 占位） |
| 11 | 小游戏交互模式 | ✅ v2.2 | **场景模式**：三底座同台、花饰预装、拖拽=移动/互换 + 咔哒；引导=三步说明条+动态提示+首次脉动 |
| 12 | 弹窗组件 | ✅ v2.2 | 左下静默卡片：8s 延迟、关闭 7 天勿扰、原生订阅成功后亮 WELCOME10；hairline+radius 0 |
| 13 | 货币 | ✅ v2.2 | **USD 默认** + 多币种自动转换（主货币切换为店主后台操作） |
